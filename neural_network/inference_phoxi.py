...
parser.add_argument('--model', default='deeplabv3plus_resnet101_depth', help='Model file name [default: votenet]')
...
parser.add_argument('--save_dir', default='/home/kb/AgileProjects/suctionnet-baseline/inference_results/deeplabV3plus',
                    help='Dump dir to save model checkpoint [default: log]')
parser.add_argument('--checkpoint_path',
                    default='/home/kb/AgileProjects/suctionnet-baseline/pretrained_models/kinect-deeplabplus-depth',
                    help='Model checkpoint path [default: None]')
parser.add_argument('--overwrite', default=True, action='store_true', help='Overwrite existing log and dump folders.')
...
def inference_one_view(rgb_file, depth_file, meta_file, scene_idx, anno_idx):
    ...
    # # ==================================================================================================================
    # print("rgb max, min: ", np.asarray(rgb).max(),np.asarray(rgb).min())
    # print("depth max, min: ", np.asarray(depth).max(), np.asarray(depth).min())
    ...
    # rgb = np.load("/home/kb/AgileProjects/suctionnet-baseline/datasets/data/rgb_img.npy").astype(np.float32) / 255.0
    # depth = np.load("/home/kb/AgileProjects/suctionnet-baseline/datasets/data/depth_img.npy").astype(np.float32)
    # depth -= 0.6
    # depth[np.where(depth<0)] = depth.max()
    # seg_mask = np.load("/home/kb/AgileProjects/suctionnet-baseline/datasets/data/mask_img.npy")
    # rgb *= single2three_channel(seg_mask)
    # depth *= seg_mask
    # depth[np.where(depth <= 0.0)] = depth.max() + 0.3
    # print("rgb max, min: ", np.asarray(rgb).max(), np.asarray(rgb).min())
    # print("depth max, min: ", np.asarray(depth).max(), np.asarray(depth).min())
    # plt.figure()
    # plt.subplot(121)
